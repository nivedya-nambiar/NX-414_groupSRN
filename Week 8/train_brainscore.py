...
def load_it_data(path_to_data):
    """ Load IT data from your H5 format
...
if __name__ == "__main__":
    ...
    path_to_data = ''  # Insert the folder where the data is
    ...
    # Print data shapes
    print(f"Train data: {stimulus_train.shape}, {spikes_train.shape}")
    print(f"Validation data: {stimulus_val.shape}, {spikes_val.shape}")
    print(f"Test data: {stimulus_test.shape}")
    
    # Visualize a sample image
    visualize_img(stimulus_train, objects_train, stim_idx=0)
    
    # Initialize the model
    num_neurons = spikes_train.shape[1]
    print(f"Number of neurons to predict: {num_neurons}")
    
    model = ITNeuralPredictor(num_neurons=num_neurons, 
                             model_name='convnext_large_mlp.clip_laion2b_augreg_ft_in1k',
                             feature_extraction_only=True)
    ...
    val_results = model.evaluate(stimulus_val, spikes_val)
    print(f"Validation Mean Correlation: {val_results['mean_correlation']:.4f}")
    print(f"Validation Mean Explained Variance: {val_results['mean_explained_variance']:.4f}")
    
    # Plot neuron metrics
    model.plot_neuron_metrics(val_results)
    
    # Save the model
    model.save_model('it_neural_predictor.pth')
    
    # Make predictions on test set (if you have ground truth for testing)
    test_predictions = model.predict(stimulus_test)
    print(f"Test predictions shape: {test_predictions.shape}")
    
    # Visualize predictions for a specific neuron
    neuron_idx = 1  # The neuron you showed in your plot
    
    plt.figure(figsize=(10, 5))
    plt.plot(spikes_val[:, neuron_idx], label='Actual')
    plt.plot(model.predict(stimulus_val)[:, neuron_idx], label='Predicted')
    plt.title(f'Actual vs Predicted Responses for Neuron {neuron_idx}')
    plt.legend()
    plt.show()